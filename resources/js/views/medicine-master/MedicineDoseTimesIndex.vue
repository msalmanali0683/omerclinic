<template>
  <div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <div>
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Dose Times</h2>
        <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">Manage medicine dose timing strings</p>
      </div>
      <BaseButton v-if="authStore.can('create medicine dose times')" @click="$router.push('/medicine-master/dose-times/create')">+ Add</BaseButton>
    </div>
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 mb-4 shadow-sm flex gap-3">
      <BaseInput v-model="search" placeholder="Search dose time..." class="flex-1" @keyup.enter="flushSearch" />
    ...
    <BaseTable :columns="columns" :rows="rows" :loading="loading">
      ...
      <template #cell-actions="{ row }">
        <div class="flex gap-1">
          <BaseButton v-if="authStore.can('edit medicine dose times')" variant="ghost" size="sm" @click="$router.push(`/medicine-master/dose-times/${row.id}/edit`)">Edit</BaseButton>
          <BaseButton v-if="authStore.can('delete medicine dose times')" variant="ghost" size="sm" @click="remove(row)">Delete</BaseButton>
        </div>
      </template>
    </BaseTable>
...
async function fetch(page = 1) {
  loading.value = true;
  try {
    const { data } = await medicineDoseTimeService.getDoseTimes({ search: search.value || undefined, page });
...
async function remove(row) {
  if (!confirm(`Delete "${row.dose_time}"?`)) return;
  try {
    await medicineDoseTimeService.deleteDoseTime(row.id);
