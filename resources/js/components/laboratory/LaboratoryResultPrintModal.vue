      ...
      <div v-if="modelValue" class="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-2 sm:p-4 no-print">
        ...
        <div class="relative w-full max-w-3xl max-h-[95vh] bg-white dark:bg-gray-900 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden">
          ...
          <div class="flex-1 overflow-y-auto px-4 pb-4 pt-0.5 sm:px-6 sm:pb-6 sm:pt-0.5 print-preview-panel">
            <div class="preview-shell overflow-auto rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-gray-950 p-3 sm:p-4">
              <div
                ...
                :style="isXrayOnlyPrint ? 'width: 210mm; min-height: 297mm; padding: 0;' : 'width: 210mm; min-height: 297mm; padding: 12mm 10mm;'"
              >
          ...
          <div class="modal-footer px-4 sm:px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3 no-print">
            ...
            <BaseButton type="button" :loading="printing" @click="handlePrint">{{ printButtonLabel }}</BaseButton>
          </div>
        </div>
      </div>
    </Transition>
...
const printResults = computed(() => {
  const results = props.printData?.laboratory_results;
  if (Array.isArray(results) && results.length) return results;
  if (props.printData?.laboratory_result) return [props.printData.laboratory_result];
  return [];
});

const isXrayOnlyPrint = computed(() => {
  const results = printResults.value;
  return results.length > 0 && results.every((result) => result.test_type === 'imaging');
});

const modalTitle = computed(() => (isXrayOnlyPrint.value ? 'X-Ray Print' : props.title));
const printButtonLabel = computed(() => (isXrayOnlyPrint.value ? 'Print X-Ray' : 'Print Report'));
