  ...
  <form class="space-y-3" @submit.prevent="addItem">
    ...
    <div class="relative">
      ...
      >
        ...
        >
          Add "{{ query.trim() }}" as new diagnosis
        </li>
      </ul>
    </div>
    <p v-if="fieldError" class="text-sm text-red-600">{{ fieldError }}</p>
    <p v-else-if="error" class="text-sm text-red-600">{{ error }}</p>
    <BaseButton type="button" :loading="addingDiagnosis" :disabled="!query.trim() || addingDiagnosis" @click="() => addItem()">
      Add Diagnosis
    </BaseButton>
  </form>
</template>
...
const props = defineProps({
  ...
  label: { type: String, default: 'Diagnosis' },
  placeholder: { type: String, default: 'Search or type diagnosis...' },
});
...
async function fetchOptions() {
  ...
  loading.value = true;
  try {
    const { data } = await diagnosisMasterService.getDiagnosisOptions({ search: term, limit: 15 });
...
function handleApiError(e, fallbackMessage) {
  ...
  fieldError.value = errors.diagnosis_text?.[0] ?? '';
...
async function addItem(force = false) {
  ...
  if (!text || addingDiagnosis.value) return;
  ...
  try {
    ...
    if (!masterId) {
      const { data: masterRes } = await diagnosisMasterService.findOrCreateDiagnosis({ diagnosis_name: text });
    ...
    const { data } = await patientVisitDiagnosisService.createVisitDiagnosis(payload);
    const diagnosis = data.data ?? data;

    toastStore.success(data.message || 'Diagnosis added successfully.');
    query.value = '';
    selectedMaster.value = null;
    options.value = [];
    emit('diagnosis-added', diagnosis);
  } catch (e) {
    if (e.response?.status === 422 && e.response?.data?.code === 'duplicate_visit_diagnosis') {
      if (confirm('This diagnosis is already on this visit. Add again anyway?')) {
        await addItem(true);
        return;
      }
      return;
    }
    handleApiError(e, 'Failed to add diagnosis.');
  } finally {
