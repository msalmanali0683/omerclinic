<?php

namespace App\Http\Requests;

use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Validation\Rule;

class UpdateLaboratoryTestTemplateRequest extends FormRequest
{
    public function authorize(): bool
    {
        /** @var \App\Models\LaboratoryTestTemplate|null $template */
        $template = $this->route('laboratory_test_template');

        return $template && $this->user()->can('update', $template);
    }

    public function rules(): array
    {
        /** @var \App\Models\LaboratoryTestTemplate $template */
        $template = $this->route('laboratory_test_template');

        return [
            'test_name'                => [
                'required', 'string', 'max:255',
                Rule::unique('laboratory_test_templates', 'test_name')->ignore($template->id),
            ],
            'test_code'                => [
                'nullable', 'string', 'max:100',
                Rule::unique('laboratory_test_templates', 'test_code')->ignore($template->id),
            ],
            'test_type'                => 'nullable|string|in:standard,imaging',
            'test_price'               => 'nullable|numeric|min:0|max:99999999.99',
            'description'              => 'nullable|string|max:2000',
            'is_active'                => 'boolean',
            'fields'                   => 'required|array|min:1',
            'fields.*.id'              => 'nullable|integer|exists:laboratory_test_template_fields,id',
            'fields.*.field_label'     => 'required|string|max:255',
            'fields.*.field_type'      => 'required|string|in:text,textarea,number,select,checkbox,date,image',
            'fields.*.unit'            => 'nullable|string|max:100',
            'fields.*.reference_range' => 'nullable|string|max:255',
            'fields.*.options'         => 'nullable|array',
            'fields.*.default_value'   => 'nullable|string|max:2000',
            'fields.*.placeholder'     => 'nullable|string|max:255',
            'fields.*.is_required'     => 'boolean',
            'fields.*.sort_order'      => 'nullable|integer|min:0',
        ];
    }
}
